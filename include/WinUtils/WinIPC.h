...
#include <io.h>  // CreateFileMappingA, OpenFileMappingA, etc.
	...
	{
	public:
		SharedMemoryReadStream(const string_t& name, const std::size_t bufferSize, const bool isPersistent) :
			_memory(ConvertString<std::string>(name), bufferSize, isPersistent)
		{
		...
		[[nodiscard]] string_t readString() const
		{
			const auto memory = static_cast<const char*>(_memory.data());
			lockForRead();
			...
			unlockRead();
			return ConvertString<string_t>(data);
		}
	...
	{
	public:
		SharedMemoryWriteStream(const string_t& name, const std::size_t bufferSize, const bool isPersistent) :
			_memory(ConvertString<std::string>(name), bufferSize, isPersistent)
		{
		...
		void write(string_view_t strView) const
		{
			const auto memory = static_cast<char*>(_memory.data());
			std::string string = ConvertString<std::string>(strView);
	...
	public:
		 ...
		 */
		SharedMemoryQueue(const string_t& name, std::uint32_t capacity,
			std::uint32_t maxMessageSize, bool isPersistent, bool isWriter)
			: _memory(ConvertString<std::string>(name), kHeaderSize + capacity * (maxMessageSize + sizeof(std::uint32_t)), isPersistent)
			, _capacity(capacity)
		 ...
		 */
		bool enqueue(string_view_t msgView)
		{
			std::string_view message = ConvertString<std::string>(msgView);
			if (!_isWriter)
			{
...
#else 
			std::string tempMessage(messageLength, '\0');
			std::memcpy(&tempMessage[0], &memory[offset + sizeof(std::uint32_t)], messageLength);
#endif
			message = ConvertString(tempMessage);
		 ...
		 * @param msg Output parameter for peeked message
		 * @return true if message was peeked, false if queue is empty
		 */
		bool peek(string_t& msg) const
		{
			std::string message = ConvertString<std::string>(msg);
			if (_isWriter)
			{
