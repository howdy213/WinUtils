...
static Logger logger(L"Injector");
...
vector<DWORD> Injector::GetProcessPIDs(const wstring& processName)
{
    ...
    if (hSnapshot == INVALID_HANDLE_VALUE) {
        logger.DLog(LogLevel::Error, L"Failed to create process snapshot");
        return pids;
    }
    ...
    if (Process32FirstW(hSnapshot, &pe32)) {
        ...
        logger.DLog(LogLevel::Error, format(L"Failed to enumerate processes, error code: {}", GetLastError()));
    }
...
bool Injector::InjectDLL(DWORD pid, const wstring& dllPath)
{
    HANDLE hProcess = OpenProcess(
        ...
        FALSE, pid);
    if (!hProcess) {
        logger.DLog(LogLevel::Error, format(L"OpenProcess failed (PID:{}) , error code: {}", pid, GetLastError()));
        return false;
    }
    ...
    if (!remoteMem) {
        logger.DLog(LogLevel::Error, format(L"VirtualAllocEx failed (PID:{}) , error code: {}", pid, GetLastError()));
        CleanupRemoteResources(hProcess, nullptr);
        return false;
    }

    if (!WriteProcessMemory(hProcess, remoteMem, dllPath.c_str(), dllPathSize, nullptr)) {
        logger.DLog(LogLevel::Error, format(L"WriteProcessMemory failed (PID:{}) , error code: {}", pid, GetLastError()));
    ...
    if (!loadLibAddr) {
        logger.DLog(LogLevel::Error, format(L"Failed to get LoadLibraryW address, error code: {}", GetLastError()));
        CleanupRemoteResources(hProcess, remoteMem);
        return false;
    }
    ...
    if (!hRemoteThread) {
        logger.DLog(LogLevel::Error, format(L"CreateRemoteThread failed (PID:{}) , error code: {}", pid, GetLastError()));
        CleanupRemoteResources(hProcess, remoteMem);
        return false;
    }
    ...
    logger.DLog(LogLevel::Info, format(L"DLL injection succeeded (PID:{})", pid));
    return true;
}
...
BOOL Injector::InjectDllViaAPC(DWORD pid, const std::wstring& dllPath)
{
...
bool Injector::InjectDLLViaAPC2(DWORD pid, const wstring& dllPath)
{
    if (!CheckPIDAlive(pid) || dllPath.empty())
    {
        logger.DLog(LogLevel::Warn, std::format(L"PID {} is invalid or DLL path is empty", pid));
        return false;
    }
    ...
    if (!hProcess)
    {
        logger.DLog(LogLevel::Error, std::format(L"OpenProcess failed (PID:{}) , error code: {}", pid, GetLastError()));
        Cleanup();
        return false;
    }
    const size_t bufSize = (dllPath.size() + 1) * sizeof(wchar_t);
    ...
    {
        logger.DLog(LogLevel::Error, std::format(L"Memory operation failed (PID:{}) , error code: {}", pid, GetLastError()));
        Cleanup();
        return false;
    }
    ...
    {
        logger.DLog(LogLevel::Error, std::format(L"Failed to get function addresses, error code: {}", GetLastError()));
        Cleanup();
        return false;
    }
    ...
    {
        logger.DLog(LogLevel::Error, std::format(L"Failed to create thread snapshot, error code: {}", GetLastError()));
        Cleanup();
        return false;
    }
    ...
    for (BOOL bRet = Thread32First(hThreadSnap, &te32); bRet; bRet = Thread32Next(hThreadSnap, &te32))
    {
        ...
        if (!hThread)
        {
            logger.DLog(LogLevel::Warn, std::format(L"OpenThread failed (TID:{}) , error code: {}", te32.th32ThreadID, GetLastError()));
            continue;
        }
        ...
        if (status == 0)
        {
            logger.DLog(LogLevel::Info, std::format(L"APC injection succeeded (TID:{})!", te32.th32ThreadID));
            injectSuccess = true;
            break;
        }
        logger.DLog(LogLevel::Error, std::format(L"NtQueueApcThreadEx failed (TID:{}) , status code: 0x{:X}", te32.th32ThreadID, status));
    }
...
{   
    if(fs::path(dllPath).filename().empty()) {
        logger.DLog(LogLevel::Error, L"Invalid DLL path");
        return;
	}
    vector<DWORD> injectedPIDs;
    logger.DLog(LogLevel::Info, format(L"Started process monitoring: {}, inject DLL: {}, check interval: {}ms", processName, dllPath, checkInterval));
    logger.DLog(LogLevel::Info, L"Press Ctrl+C to exit...");
    ...
    while (true) {
        ...
        for (DWORD pid : currentPIDs) {
            if (find(injectedPIDs.begin(), injectedPIDs.end(), pid) == injectedPIDs.end()) {
                logger.DLog(LogLevel::Info, format(L"Detected new process (PID:{}) , attempting injection...", pid));
                if (InjectDLL(pid, dllPath)) {
                    injectedPIDs.push_back(pid);
                }
                else {
                    logger.DLog(LogLevel::Error, format(L"Injection failed (PID:{})", pid));
                }
            }
        }
...
vector<pair<HMODULE, wstring>> Injector::GetProcessModules(DWORD pid)
{
    ...
    if (hSnapshot == INVALID_HANDLE_VALUE) {
        logger.DLog(LogLevel::Error, format(L"Failed to create module snapshot (PID:{}) , error code: {}", pid, GetLastError()));
        return modules;
    }
    ...
    if (Module32FirstW(hSnapshot, &me32)) {
        ...
        logger.DLog(LogLevel::Error, format(L"Failed to enumerate modules (PID:{}) , error code: {}", pid, GetLastError()));
    }
...
bool Injector::UninjectDLL(DWORD pid, const wstring& dllPath)
{
    ...
    if (!targetModule) {
        logger.DLog(LogLevel::Error, format(L"Target DLL not found (PID:{}) : {}", pid, dllFileName));
        CloseHandle(hProcess);
        return false;
    }
    ...
    if (!freeLibAddr) {
        logger.DLog(LogLevel::Error, format(L"Failed to get FreeLibrary address, error code: {}", GetLastError()));
        CloseHandle(hProcess);
        return false;
    }
    ...
    logger.DLog(LogLevel::Info, format(L"DLL uninjection succeeded (PID:{}) : {}", pid, dllFileName));
    return true;
}
    ...
    const vector<DWORD>& excludeProcess)
{
    ...
    HANDLE hSnapshot = CreateProcessSnapshot();
    if (hSnapshot == INVALID_HANDLE_VALUE) {
        logger.DLog(LogLevel::Error, format(L"Failed to create process snapshot, error code: {}", GetLastError()));
        return successPIDs;
    }
    ...
    if (Process32FirstW(hSnapshot, &pe32)) {
        do {
            ...
            logger.DLog(LogLevel::Info, format(L"Attempting to inject process: {} (PID:{})", pe32.szExeFile, pe32.th32ProcessID));
            if (InjectDLL(pe32.th32ProcessID, dllPath)) {
                successPIDs.push_back(pe32.th32ProcessID);
                logger.DLog(LogLevel::Info, format(L"Injection succeeded: {} (PID:{})", pe32.szExeFile, pe32.th32ProcessID));
            }
            else logger.DLog(LogLevel::Error, format(L"Injection failed: {} (PID:{})", pe32.szExeFile, pe32.th32ProcessID));
        } while (Process32NextW(hSnapshot, &pe32));
    }
    else {
        logger.DLog(LogLevel::Error, format(L"Failed to enumerate processes, error code: {}", GetLastError()));
    ...
    const vector<DWORD>& excludeProcess)
{
    ...
    if (Process32FirstW(hSnapshot, &pe32)) {
        do {
            ...
            if (ToLower(pe32.szExeFile) != targetProcess) continue;
            logger.DLog(LogLevel::Info, format(L"Attempting to uninject DLL from process: {} (PID:{})", pe32.szExeFile, pe32.th32ProcessID));
            if (UninjectDLL(pe32.th32ProcessID, dllPath)) {
                successPIDs.push_back(pe32.th32ProcessID);
            }
            else  logger.DLog(LogLevel::Error, format(L"Uninjection failed: {} (PID:{})", pe32.szExeFile, pe32.th32ProcessID));
        } while (Process32NextW(hSnapshot, &pe32));
    }
    else {
        logger.DLog(LogLevel::Error, format(L"Failed to enumerate processes, error code: {}", GetLastError()));
